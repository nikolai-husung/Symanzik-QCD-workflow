...
id vert(aquark(p?,spt1?,cf1?,fl1?),`Q'(q?,spt2?,cf2?,fl2?),field1?{gluon,bgf}(r?,gamma?,b?,fl5?),src(?args)) =
   + TC(b,cf1,cf2)*M(`Q')*FL(fl1,fl2)*d_(gamma,kappa)*g
   + TC(b,cf1,cf2)*M(quark)*FL(fl1,fl2)*d_(gamma,kappa)*g
   - TC(b,cf1,cf2)*FL(fl1,p,fl2)*d_(gamma,kappa)*i_*g
   + TC(b,cf1,cf2)*FL(fl1,q,fl2)*d_(gamma,kappa)*i_*g
   - TC(b,cf1,cf2)*FL(fl1,gamma,fl2)*p(kappa)*i_*g
   + TC(b,cf1,cf2)*FL(fl1,gamma,fl2)*q(kappa)*i_*g;
...
#do dummy=1,1
   id,once vert(aquark(p?,spt1?,cf1?,fl1?),`Q'(q?,spt2?,cf2?,fl2?),field1?{gluon,bgf}(r?,gamma?,b?,fl5?),field2?{gluon,bgf}(s?,delta?,c?,fl6?),src(?args)) =
      + TC(b,cf1,cf`cfcnt')*TC(c,cf`cfcnt',cf2)*FL(fl1,gamma,fl2)*d_(delta,kappa)*g^2
      + TC(b,cf1,cf`cfcnt')*TC(c,cf`cfcnt',cf2)*FL(fl1,delta,fl2)*d_(gamma,kappa)*g^2
      + TC(b,cf`cfcnt',cf2)*TC(c,cf1,cf`cfcnt')*FL(fl1,gamma,fl2)*d_(delta,kappa)*g^2
      + TC(b,cf`cfcnt',cf2)*TC(c,cf1,cf`cfcnt')*FL(fl1,delta,fl2)*d_(gamma,kappa)*g^2;
