id vert(field1?{gluon,bgf}(p?,alpha?,b?,fl1?),field2?{gluon,bgf}(q?,beta?,c?,fl2?),src(?args)) =
   -i_*(p(kappa)+q(kappa))*4*e_(p,q,alpha,beta)*d_(b,c);


id vert(field1?{gluon,bgf}(p?,alpha?,b?,fl1?),field2?{gluon,bgf}(q?,beta?,c?,fl2?),field3?{gluon,bgf}(r?,gamma?,d?,fl3?),src(?args)) = i_*(p(kappa)+q(kappa)+r(kappa))*
   (- 4*FC(b,c,d)*e_(p,alpha,beta,gamma)*i_*g
   - 4*FC(b,c,d)*e_(q,alpha,beta,gamma)*i_*g
   - 4*FC(b,c,d)*e_(r,alpha,beta,gamma)*i_*g);

#do dummy=1,1
   id,once vert(field1?{gluon,bgf}(p?,alpha?,b?,fl1?),field2?{gluon,bgf}(q?,beta?,c?,fl2?),field3?{gluon,bgf}(r?,gamma?,d?,fl3?),field4?{gluon,bgf}(s?,delta?,e?,fl4?),src(?args)) = i_*(p(kappa)+q(kappa)+r(kappa)+s(kappa))*
