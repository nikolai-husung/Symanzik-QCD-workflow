...
#do dummy=1,1
   id,once vert(aquark(p?,alpha?,cf1?,fl1?),`Q'(q?,beta?,cf2?,fl2?),field3?{gluon,bgf}(r?,gamma?,b?,fl3?),src(?args)) =
      + 2*TC(b,cf1,cf2)*FL(fl1,spt`sptcnt',fl2)*e_(r,gamma,kappa,spt`sptcnt')*i_*g*OPF(aquark,`Q',fl1);
...
#do dummy=1,1
   id,once vert(aquark(p?,alpha?,cf1?,fl1?),`Q'(q?,beta?,cf2?,fl2?),field3?{gluon,bgf}(r?,gamma?,b?,fl3?),field4?{gluon,bgf}(s?,delta?,c?,fl4?),src(?args)) =
      + 2*FC(b,c,col`ccnt')*TC(col`ccnt',cf1,cf2)*FL(fl1,spt`sptcnt',fl2)*e_(gamma,delta,kappa,spt`sptcnt')*g^2*OPF(aquark,`Q',fl1);
